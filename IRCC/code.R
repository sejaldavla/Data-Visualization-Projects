...
ggsave("p1.png", width = 2, height = 2, units = "in", dpi = 300)
...
p9 <- plot_circular(2014) +
  geom_richtext(aes(y = Enrolment, 
                    label = scales::comma(Enrolment)),
                fill = NA,
                family = "Alegreya Sans SC",
                angle = c(240, 310, 330, 333, 337, 340, 345, 350, 355, 355),
                hjust = 0,
                label.color = NA) +
  annotate(geom = "text",
           label = " 2014",
           ...
           family = "Lilita One") +
  annotate(geom = "text",
           label = "Total Incoming students - 330,112",
           x = -5, 
           y = 0,
           color = "black",
           size = 5,
...
p10 <- plot_circular(2013) +
  geom_richtext(aes(y = Enrolment, 
                    label = scales::comma(Enrolment)),
                fill = NA,
                family = "Alegreya Sans SC",
                angle = c(265, 310, 330, 333, 337, 340, 345, 350, 355, 355),
                hjust = 0,
                label.color = NA) +
  annotate(geom = "text",
           label = " 2013",
           ...
           family = "Lilita One") +
  annotate(geom = "text",
           label = "Total Incoming students - 301,542",
           x = -5, 
           y = 0,
           color = "black",
           size = 5,
...
L1_subtitle <- "Ranking newly arrived international student population by top 10 nationalities shows that majority of students in the last 5 <span style='color:#845EC2'>years</span> come from <span style='color:#00429d'>**India**</span>."

L1 <- p10 + p9 + p8 + p7 + p6 + p5 + p4 + p3 + p2 + p1 +
  plot_layout(ncol = 5) +
...
ggsave("L1.png", width = 11, height = 5, units = "in", dpi = 300)

####----------------------------------------------------------------------------------------------------------

L2_title <- ggplot() +
  annotate("text", x = 0.5, y = 1, label = L1_title, vjust = 1, lineheight = 0.3, family = "Lilita One", color = "#D9544D", fontface = "bold", size = 20) +
  xlim(0, 1) +
  ylim(0, 1) +
...
L2 <- ggplot() +
  inset_element(p10, left = -0.1, right = 0.4, top = 0.9, bottom = 0.4) +
  inset_element(p9, left = 0.1, right = 0.6, top = 0.9, bottom = 0.4) +
  inset_element(p8, left = 0.3, right = 0.8, top = 0.9, bottom = 0.4) +
  inset_element(p7, left = 0.5, right = 1.0, top = 0.9, bottom = 0.4) +
  inset_element(p6, left = 0.7, right = 1.1, top = 0.9, bottom = 0.4)

ggsave("L2.png", width = 10, height = 4, units = "in", dpi = 300)
...
ircc_top |>
  ggplot(aes(x = Year, y = Enrolment, fill = Country)) +
  geom_stream() +
